...
async function createOrder(data: { values: FormValues; images: File[] }) {
  ...
  if (images.length > 0) {
    ...
    await fetchMediaWithAuth("/order/images", orderImagesSchema, {
      init: {
        method: "POST",
        body: formData,
      },
...
export function ComprehensiveMovingForm() {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    mutate({ values, images: selectedImages });
  });
